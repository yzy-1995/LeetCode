...
public class maximumprofitofoperatingacentennialwheel {
    public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        ...
        if (4 * boardingCost <= runningCost)
            return -1;
        int ans = -1;  // 答案
        int max = 0, tmp = 0;  // 最大利润、本轮的利润
        int wait = 0;  // 等待的乘客数
        for(int i = 0; i < customers.length || wait > 0;) {  // i是当前轮数
            // 如果customers遍历完了，就处理剩下在wait的乘客
            wait += i < customers.length ? customers[i] : 0;
            int up = Math.min(4, wait);  // 一次最多容纳的乘客
            wait -= up;
            ++i;
            tmp += up * boardingCost - runningCost;
            if (tmp > max) {  // 当前值更大，记录当前轮数和当前利润
                max = tmp;
                ans = i;
            }
        }
