 ...
 * 输出：10
 * 解释： 由于单次最多采集 3 颗 第 0 批需要采集 2 颗第 0 类型果实，需要采集 1 次，耗时为 2*1=2 第 1 批需要采集 4 颗第 1 类型果实，
 * 需要采集 2 次，耗时为 3*2=6 第 2 批需要采集 1 颗第 2 类型果实，需要采集 1 次，耗时为 2*1=2 返回总耗时 2+6+2=10
 * 示例 2：
 * 输入：time = [1], fruits = [[0,3],[0,5]], limit = 2
 * 输出：5
 * 解释： 由于单次最多采集 2 颗 第 0 批需要采集 3 颗第 0 类型果实，需要采集 2 次，耗时为 1*2=2 第 1 批需要采集 5 颗第 0 类型果实，
 * 需要采集 3 次，耗时为 1*3=3 需按照顺序依次采集，返回 2+3=5
...
public class PTXy4P {
    ...
    public int getMinimumTime(int[] time, int[][] fruits, int limit) {
        ...
        int n = fruits.length;
        for(int i = 0; i < n; i++){
            num = fruits[i][1] % limit == 0 ? fruits[i][1] / limit : fruits[i][1] / limit + 1;
            sum += time[fruits[i][0]] * num;
        }
        return sum;
    }
}
