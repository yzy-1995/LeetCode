...
/**
 * ClassName: NumberofValidWordsinaSentence
 * Description:
...
public class NumberofValidWordsinaSentence {
    ...
    private boolean isValidWord(String token) {
        ...
        if (hasLetter && hyphenCount <= 1 && punctuationCount <= 1) {
            if (hyphenCount == 1) {
                int hyphenIndex = token.indexOf('-');
                return Character.isLowerCase(token.charAt(hyphenIndex - 1)) && Character.isLowerCase(token.charAt(hyphenIndex + 1));
            }
            return true;
        }
